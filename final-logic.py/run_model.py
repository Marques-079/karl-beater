...
try:
    while True:
        ...
        if not run_detection and key == ord("p"):
            ...
            screen_x = BBOX["left"] + px
            screen_y = BBOX["top"]  + py - 720    # your calibrated offset

            # position cursor over paddle (no hold yet)
        ...
        # ── 3) Predict where it’ll land at the slider-top line ───────────
        ...
        # ── 4) Drag paddle to the predicted X in one smooth hold ────────
        new_screen_x = BBOX["left"] + final_x
        pyautogui.dragTo(new_screen_x, screen_y, duration=0.2, button='left')

except KeyboardInterrupt:
    pass

finally:
    # ensure any held mouse is released
    if run_detection:
        pyautogui.mouseUp()
